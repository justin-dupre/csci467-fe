    ...
    processOrder(quote) {
        ...
        axios.post(url, postData, config)
            .then(res => {
                ...
                window.alert('Order ' + res.data.order + ' processed and commission of $' + ((parseFloat(res.data.commission) / 100.0) * quote.price).toFixed(2) + ' has been added to associate. Email has been sent to customer. Processed on ' + s + ' at ' + t + ' by ' + this.props.associate.name)
                ...
                <table class="table table-striped mt-5">
                    ...
                    <tbody>
                        {sanctionedQuotes.map((quote, i) => {
                            return (
                                <tr >
                                    <th className="truncate" scope="row">#{quote.id}</th>
                                    ...
                                    <td className="truncate">{quote.processed ? <img style={{ width: '25px' }} src={check} /> : <img onClick={() => this.processOrder(quote)} style={{ width: '25px' }} src={cart} />}</td>
                                </tr>
                            );
                        })}
                    </tbody>
